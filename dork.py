...
class Dorker(object):
    ...
    def dork(self, search):
        ...
        """
        if self._engine == 'duck' or not self._engine: # seems hopeless at 20-02-2011 -> 19-12-2016
            search_url = "https://duckduckgo.com/html/?q=" + '"' + urllib2.quote(search) + '"'

        elif self._engine == 'bing': # works at 20-02-2011 -> 19-12-2016
            search_url = "https://www.bing.com/search?q=" + 'instreamset:(url):"' + urllib2.quote(search) + '"'

        elif self._engine == 'google': # works at 11/11/2011 -> 26-12-2016
            search_url = "https://www.google.com/xhtml?q=" + 'inurl:"' + urllib2.quote(search) + '"'

        elif self._engine == 'yahoo': # works at 20-02-2011 -> 19-12-2016
            search_url = "https://search.yahoo.com/search?q=" + 'instreamset:(url):"' + urllib2.quote(search) + '"'

        elif self._engine == 'yandex': # works at 20-02-2011 -> 19-12-2016
        ...
        if self._engine == 'duck':
            regex = '<a rel="nofollow" class="large" href="(.+?)"' # regex magics 20-12/2016
        if self._engine == 'bing':
            regex = '<li class="b_algo"><h2><a href="(.+?)">' # regex magics 20-12/2016
        if self._engine == 'google':
            regex = '<h3 class="r"><a href="/url(.+?)">' # regex magics 20-12/2016
        if self._engine == 'yahoo':
            regex = '<h3 class="title"><a class=" ac-algo ac-21th lh-15" href="(.+?)">' # regex magics 20-12/2016
        if self._engine == 'yandex':
            regex = '<a class="link serp-item__title-link" target="_blank" href="(.+?)"' # regex magics 20-12/2016
